...
class BaseAggregator(AbstractAnnotator):
    ...
    def __call__(self, doc: Doc) -> Doc:
        ...
        if len(sources) > 0:
            ...
            if "O" in self.out_labels:
                ...
                output_probs = {
                    span: {label: prob for label, prob in distrib.items() if prob > 0.1}
    ...
    def get_observation_df(self, doc: Doc):
        ...
        # Otherwise, returns a dataframe with span-level predictions
            ...
            unique_spans = set(
                (span.start, span.end) for s in sources for span in doc.spans[s]
            )
            sorted_spans = sorted(unique_spans)
            spans_indices = {span: i for i, span in enumerate(sorted_spans)}

            data = np.full(
                (len(unique_spans), len(sources)), fill_value=0, dtype=np.int16
            )
            ...
            sorted_unique_spans = [
                span for i, span in enumerate(sorted_spans) if masking[i]
            ]
    ...
    def add_underspecified_label(
        self, underspec_label: str, satisfied_values: Set[str]
    ):
    ...
    def get_underspecification_matrix(self) -> np.ndarray:
        ...
        matrix = np.zeros(
            (len(self.underspecified_labels), len(self.out_labels)), dtype=bool
        )
...
class MajorityVoter(BaseAggregator):
    ...
    def _aggregate(self, obs: pandas.DataFrame, coefficient=0.1) -> pandas.DataFrame:
        ...
        def count_function(x):
            ...
            weights = [
                self.weights.get(source, 1) for source in obs.columns[x >= min_val]
            ]
            return np.bincount(
                x[x >= min_val] - min_val, weights=weights, minlength=nb_obs_to_count
            )
...
class HMM(hmmlearn.base._BaseHMM, BaseAggregator):
    ...
    def _aggregate(self, obs: pandas.DataFrame) -> pandas.DataFrame:
        ...
        # Run a forward pass
        _, forward_lattice = self._do_forward_log_pass(framelogprob)
    ...
    def fit(self, docs: Iterable[Doc], cutoff: int = None, n_iter=5, tol=1e-2):
        ...
        for iter in range(n_iter):
            ...
            for doc_i, doc in enumerate(docs):
                ...
                if (
                    np.isnan(framelogprob).any()
                    or framelogprob.max(axis=1).min() < -100000  # type: ignore
                ):
    ...
    def _compute_log_likelihood(self, X: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        for source in X:
            ...
            probs = self.emit_probs[source] ** self.weights.get(  # type:ignore
                source, 1
            )
    ...
    def _reset_counts(self, sources, fp_prior=0.1, fn_prior=0.3, concentration=1):
        ...
        self.emit_counts = {
            source: np.zeros(shape=(nb_labels, nb_obs)) for source in sources
        }
        ...
        for source in sources:
            if "O" in self.out_labels:
                self.emit_counts[source][:, :nb_labels] = (
                    np.eye(nb_labels) * concentration
                )
                self.emit_counts[source][0, :] += fp_prior * concentration
            else:
                self.emit_counts[source][:, 1 : nb_labels + 1] = (
                    np.eye(nb_labels) * concentration
                )
    ...
    def _get_correlated_sources(self, source, all_sources):
        ...
        for other_source in all_sources:
            ...
            if source2 == other_source2:
                ...
                source2.startswith(other_source2)
                or source2.endswith(other_source2)
                or other_source2.startswith(source2)
                or other_source2.endswith(source2)
            ):
                corr_sources.append(other_source)
        return corr_sources
    ...
    def _add_mv_counts(self, docs: Iterable[Doc]):
        ...
        init_mv_weights = {
            source: self.initial_weights.get(source, 1)
            * np.exp(
                -self.redundancy_factor
                * len(  # type: ignore
                    [s2 for s1, s2 in self.corr_counts if s1 == source]
                )
            )
            for source in self.emit_counts
        }
        ...
        for doc in docs:
            ...
            for src, src2 in self.corr_counts:
                if src in one_hots and src2 in one_hots:
                    self.corr_counts[(src, src2)] += np.dot(
                        one_hots[src2].T, one_hots[src]
                    )
    ...
    def _accumulate_statistics(
        ...
        if n_samples > 1:
            ...
            hmmlearn._hmmc._compute_log_xi_sum(
                n_samples,
                n_components,
                fwdlattice,  # type: ignore
    ...
    def _do_mstep(self):
        ...
        for source in self.emit_counts:
            normalisation = (self.emit_counts[source] + 1e-100).sum(axis=-1)[
                :, np.newaxis
            ]
    ...
    def _update_weights(self):
        ...
        for source in self.emit_counts:
            ...
            self.weights[source] = np.full(
                fill_value=init_weight,
        ...
        for source in self.emit_counts:
            for i in range(len(self.observed_labels)):
                ...
                self.weights[source][i] *= np.exp(
                    -self.redundancy_factor
                    * np.sum(recalls_with_corr_sources)  # type: ignore
                )
    ...
    def _postprocess_counts(self):
        ...
        for emit_counts in self.emit_counts.values():
            ...
            new_counts = (
                0.1 * cur_counts
                + 0.9 * cur_counts * self.get_underspecification_matrix().T
            )
    ...
    def pretty_print(self, sources=None, nb_digits=2, show_counts=False):
        ...
        if hasattr(self, "startprob_"):
            ...
            print(
                pandas.Series(self.startprob_, index=self.out_labels).round(nb_digits)
            )
            print("--------")
        if hasattr(self, "transmat_"):
            ...
            print(
                pandas.DataFrame(
                    self.transmat_, index=self.out_labels, columns=self.out_labels
                ).round(nb_digits)
            )
            print("--------")
        for source in sorted(self.emit_counts):
            ...
            if sources == None or source in sources:
                ...
                df = pandas.DataFrame(
                    vals, index=self.out_labels, columns=self.observed_labels
                )
                if self.weights:
                    dft = df.transpose()
